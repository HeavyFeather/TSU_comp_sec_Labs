...
// extension p-o Pollard


void ftof1(BigNum& x, BigNum& u, BigNum& v, BigNum g, BigNum a, BigNum n, BigNum m) {
		if (x % (BASE)3 == (BASE)1)
		{
			x = x * a % n;
			v = (v + (BASE)1) % m;
...
BigNum BigNum::polard3(BigNum g, BigNum a) {
	...
	do {
		ftof1(x1, u1, v1, g, a, n, m);
		ftof1(x2, u2, v2, g, a, n, m);
		ftof1(x2, u2, v2, g, a, n, m);
	} while (x1 != x2);
	...
	r /= d;
	m /= d;
	Y /= d; // делим всё на d, чтобы были взаимнопростые числа
